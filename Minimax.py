...
def minimax_pruned(board, depth, alpha, beta, maximizingPlayer):
    ...
    if maximizingPlayer:
        ...
        for col in valid_moves:
            board_copy = board.copy()
            drop_piece(board_copy, get_next_open_row(board_copy, col), col, PLAYER_PIECE) #simulate dropping a piece
...
#---------- neutral minimax with alpha-beta pruning ----------
#it's only a few extra lines but i thought i should separate this too
def minimax_pruned_neutral(board, depth, alpha, beta, maximizingPlayer, piece):
    valid_moves = get_valid_moves(board)
    is_terminal = is_terminal_node(board)
    opp = get_opponent(piece)
    if depth == 0 or is_terminal: #can it be processed?
        if is_terminal:
            if winning_move(board, piece): #high score for bot winning
                return (None, 123456789)
            elif winning_move(board, opp): #low score for human winning
                return (None, -123456789)
            else: #no more valid moves (tie)
                return (None, 0)
        else: #depth == zero
    ...
    if maximizingPlayer:
        ...
        for col in valid_moves:
            ...
            new_score = minimax_pruned_neutral(board_copy, depth - 1, alpha, beta, False, piece)[1]
            if new_score > minimax_score:
                minimax_score = new_score
                column = col
            alpha = max(alpha, minimax_score)
        ...
        for col in valid_moves:
            board_copy = board.copy()
            drop_piece(board_copy, get_next_open_row(board_copy, col), col, opp) #simulate dropping a piece
            new_score = minimax_pruned_neutral(board_copy, depth - 1, alpha, beta, True, piece)[1]
            if new_score < minimax_score:
                minimax_score = new_score
                column = col
            beta = min(beta, minimax_score)
